	...
	<div class="panel panel-default">
	    ...
	    <div class="panel-body">
	    	<form id="search-form">
				<div class="col-sm-4 my-from-group">
					...
					<input type="text" name="startTime" class="form-control" value='<fmt:formatDate value="${diamondIncomeQuery.startTime}" pattern="yyyy-MM-dd HH:mm:ss"/>' onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd HH:mm:ss'})" placeholder="开始时间" >
				</div>
				<div class="col-sm-4 my-from-group">
					<label class="control-label">结束时间</label>
					<input type="text" name="endTime" class="form-control" value='<fmt:formatDate value="${diamondIncomeQuery.endTime}" pattern="yyyy-MM-dd HH:mm:ss"/>' onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd HH:mm:ss'})" placeholder="结束时间" >
				</div>
				<div class="col-sm-4 my-from-group">
					<label class="control-label">用户编号</label>
					<input type="number" name="buyerId" class="form-control" value="${fn:escapeXml(diamondIncomQuery.buyerId)}" placeholder="请输入用户编号">
				</div>
				<div class="col-sm-4">
		...
		<table class="table table-bordered my-table">
			...
			<tr>
				...
				<td>${fn:escapeXml(element.buyerName)}</td>
				<td>${element.buyDiamond}</td>
				<td>${element.buyMoney}元</td>
				<td><fmt:formatDate value="${element.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				<td>${element.income}</td>
				<td>${element.rate}</td>
				<td>${element.downLevel}</td>
			</tr>
			</c:forEach>
		</table>
	</div>
