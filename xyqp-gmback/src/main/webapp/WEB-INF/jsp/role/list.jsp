	...
	<div class="panel panel-default">
	    ...
	    <div class="panel-body">
	    	<form id="search-form">
				<div class="col-sm-4 my-from-group">
					...
					<input type="text" name="id" class="form-control" value="${fn:escapeXml(roleQuery.id)}" placeholder="请输入编号">
				</div>
				<div class="col-sm-4 my-from-group">
					<label class="control-label">名称</label>
					<input type="text" name="name" class="form-control" value="${fn:escapeXml(roleQuery.name)}" placeholder="请输入名称">
				</div>
				<div class="col-sm-4">
		...
		<table class="table table-bordered my-table">
			...
			<tr>
				...
				<td>${element.name}</td>
				<td>
					<sys:permission url="/role/delete">
						<span class="js-delete my-action glyphicon glyphicon-trash" data-id="${element.id}" title="删除"></span>
					</sys:permission>
					<sys:permission url="/role/update">
						<span class="js-update my-action glyphicon glyphicon-edit" data-id="${element.id}" title="修改"></span>
					</sys:permission>
					<sys:permission url="/roleModule/allot">
						<span class="js-allot my-action glyphicon glyphicon-cog" data-id="${element.id}" title="分配权限"></span>
					</sys:permission>
				</td>
			</tr>
			</c:forEach>
...
$(".js-allot").click(function(){
	var id = $(this).data("id");
	location.href = "../roleModule/allot?roleId=" + id;
});
</script>
</html>
